...
export default function AsistenciaPage() {
  ...
  const renderIcon = () => {
    ...
    if (data.message?.toLowerCase().includes('ya registró')) {
      return <Clock className="text-yellow-500 w-12 h-12 mb-2" />
    }
            ...
            {data.success && (
              <>
                <p className="text-gray-700 text-2xl mb-4">
                  {data.clasesRealizadas} de {data.clasesPagadas} clases realizadas
                </p>
                <div className="w-full bg-gray-300 rounded-full h-4 overflow-hidden mb-4">
                  <div className="bg-orange-500 h-full transition-all duration-500" style={{ width: `${porcentaje}%` }}></div>
                </div>
                <div className="text-left text-gray-800 space-y-2 mb-4">
                  <p className="flex items-center justify-between gap-2 text-2xl">
                    <strong className="flex items-center gap-1 text-orange-600">
                      <BadgeCheck className="w-7 h-7" /> Plan:
                    </strong>
                    {data.plan}
                  </p>
                  <p className="flex items-center justify-between gap-2 text-2xl">
                    <strong className="flex items-center gap-1 text-orange-600">
                      <CalendarCheck className="w-7 h-7" /> Vence:
                    </strong>
                    {data.fechaVencimiento}
                  </p>
                  <p className="flex items-center justify-between gap-2 text-2xl">
                    <strong className="flex items-center gap-1 text-orange-600">
                      <Coins className="w-7 h-7" /> GymspaceCoins:
                    </strong>
                    {data.gymCoins}
                  </p>
                </div>
